...
def set_isolated_logger_configs(kubernetes_pod_operator_kwargs: dict):
...
def set_default_namespace_configs(kubernetes_pod_operator_kwargs: dict):
    if 'namespace' not in kubernetes_pod_operator_kwargs:
        kubernetes_pod_operator_kwargs["namespace"] = conf.get('kubernetes', 'NAMESPACE')


def remove_unsettable_kubernetes_pod_operator_kwargs(kubernetes_pod_operator_kwargs):
...
def render_airflow_templates(operator, args, kwargs, context):
    ...
    # Notes for Context
    # https://github.com/apache/airflow/blob/main/airflow/models/taskinstance.py#L2175-L2222
...
def set_simple_templates_as_env_vars(args, kwargs, kubernetes_pod_operator_kwargs):
    ...
    >>> import os; os.environ['AIRFLOW_VAR_FOO'] = 'bar'
    >>> set_simple_templates_as_env_vars(["{{ var.value.foo }}"], {}, {})
    ...
    def _set_conns_from_values(_value):
        ...
        if results:
            for conn_id in results:
                ...
                logging.debug(
                    f'Passing connection {conn_id} through to IsolatedOperator via ENV VAR'
                )
                env_vars[f'AIRFLOW_CONN_{conn_id.upper()}'] = Connection \
                    .get_connection_from_secrets(conn_id=conn_id) \
                    .get_uri()

    def _set_conns_from_key_and_values(_key, _value):
        if type(_value) != str or type(_key) != str:
            return
        ...
        for results in [pattern.findall(_value) for pattern in var_template_patterns]:
            if results:
                for var_id in results:
                    logging.debug(
                        f'Passing variable {var_id} through to IsolatedOperator via ENV VAR'
                    )
                    env_vars[f'AIRFLOW_VAR_{var_id.upper()}'] = Variable.get(var_id)

    # Iterate over *args
    for i, val in enumerate(args):
        logging.info(f"Checking args[{i}]={val} for vars and conns...")
        if val:
            # look for {{ var.xyz }}
    ...
    for key, val in kwargs.items():
        logging.info(f"Checking {key}={val} ...")
        if val:
            # look for {{ var.xyz }}
            _set_vars_from_values(val)
...
class IsolatedOperator(KubernetesPodOperator):
    # TODO - ExternalPython as well and docker via __new__
    isolated_operator_args = ["task_id", "image", "cmds", "arguments", "log_events_on_failure"]
    ...
    def __init__(
        ...
        super().__init__(
            ...
            image=image,
            cmds=["python"],
            arguments=["-c", create_isolated_operator_command(task_id, self.operator, *self._args, **self._kwargs)],
